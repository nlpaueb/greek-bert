...
from .model import UDBERTModel
from .dataset import UDBERTDataset


class UDBERTSystemWrapper:
    ...
    def __init__(self, pretrained_bert_name, model_params):
        ...
        model = UDBERTModel(bert_model, **model_params)
    ...
    def train(self, train_dataset_file, val_dataset_file, lr, batch_size, grad_accumulation_steps, run_on_multi_gpus):
        ...
        train_dataset = UDBERTDataset(train_dataset_file, tokenizer)
        val_dataset = UDBERTDataset(val_dataset_file, tokenizer)
    ...
    def _train_impl(self,
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=UDBERTDataset.collate_fn
        )
    ...
    def evaluate(self, eval_dataset_file, batch_size, run_on_multi_gpus):
        tokenizer = AutoTokenizer.from_pretrained(self._pretrained_bert_name)
        eval_dataset = UDBERTDataset(eval_dataset_file, tokenizer)
    ...
    def _evaluate_impl(self, eval_dataset, batch_size, run_on_multi_gpus):
        ...
        evals = {
            'acc': evaluators.MaskedTokenLabelingEvaluatorWrapper(pw.evaluators.MultiClassAccuracyEvaluator()),
            'macro-prec': evaluators.MaskedTokenLabelingEvaluatorWrapper(
                pw.evaluators.MultiClassPrecisionEvaluator(average='macro')
            ),
            'macro-rec': evaluators.MaskedTokenLabelingEvaluatorWrapper(
    ...
    def tune(pretrained_bert_name, train_dataset_file, val_dataset_file, run_on_multi_gpus):
        ...
        for i, (lr, dp, grad_accumulation_steps) in enumerate(params):
            ...
            torch.manual_seed(0)
            current_system_wrapper = UDBERTSystemWrapper(pretrained_bert_name, {'dp': dp})
